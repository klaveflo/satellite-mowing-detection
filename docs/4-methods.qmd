...
To quantify the effect of both the raw and SNR-corrected coherence on class separability, Cohen's $d$ effect sizes and Mann-Whitney p-values were computed between mowed and unmowed samples for each polarisation and seasonal window. The growing season was divided into three periods: early (April to May), peak (June to July), and late (August to October). Based on the quadruplet design described above, a positive Cohen's $d$ indicates that mowed parcels show higher post-event coherence than unmowed parcels. The results of this analysis are reported in @sec-results.
