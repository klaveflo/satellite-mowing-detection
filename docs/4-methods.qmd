...
To support the practical application of the developed methodology, a standalone prototype was implemented as an interactive Jupyter Notebook. This tool is designed for end-users, such as environmental researchers or land managers, allowing them to apply the automated mowing detection workflow with minimal programming expertise. The prototype exclusively uses one of the optical `S2-only` model configuration. The integration of Synthetic Aperture Radar data was omitted from this standalone tool because the high computational overhead, data volume constraints, and complex preprocessing pipelines associated with interferometric coherence estimation would severely limit its accessibility as a lightweight, user-friendly application.

**The notebook is designed to require only minimal adjustments from the user, such as defining the geographic boundaries of the target area and the desired observation timeframe. It then automates the retrieval of Sentinel-2 imagery, the computation of the required spectral difference features, and the application of the pre-trained classification model.** **[NOTE: NOT CORRECT, currently the notebook does not automatically retrieve Sentinel-2 imagery, but rather requires the user to provide pre-processed input rasters. if time allows will, nb will be adjusted accordingly, otherwise change text!!]**

Within the scope of this study, this prototype is explicitly used to conduct a spatial generalisation test. By applying the tool to external grassland patches located entirely outside the primary Zurich Airport study site, the transferability and robustness of the optical baseline model on unknown geographic areas are evaluated. The qualitative findings of this generalisation test are discussed in @sec-results. [**Explain where exactly it was applied!**]